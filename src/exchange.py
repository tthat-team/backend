...
def get_exchange_rate(base_currency,currency1):
    
    today = date.today()
    # modifies the url to access text from api, then stores the rate info
    
    url = 'https://api.exchangerate.host/timeseries?start_date='+str(today)+'&end_date='+str(today)+'&base='+base_currency+'&symbols='+currency1
    jsonRates= requests.get(url)
    jsonObject = json.loads(jsonRates.text)
    rates = jsonObject["rates"]
    
    value = rates[str(today)]
    ex_rate = value[currency1]
    return ex_rate
